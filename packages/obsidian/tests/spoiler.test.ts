...
test('Spoiler Detection & Live Preview', async (t) => {
  ...
  await t.test('excludes fenced code block content', () => {
    ...
    const ranges = findSpoilerRanges(text)
    assert.strictEqual(ranges.length, 1)
    assert.strictEqual(text.slice(ranges[0].from, ranges[0].to), '||actual spoiler||')
  })
  ...
  await t.test('isRangeInCodeNode returns false when no syntax tree is present', () => {
  ...
  await t.test('getSpoilerFragments generates single fragment with start and end caps', () => {
  ...
  await t.test('buildSpoilerDecorations creates decorations with cap classes and data-spoiler-id', () => {
    const docText = 'Hello ||secret|| world'
    ...
    assert.strictEqual(contentDeco.spec.attributes['data-spoiler-id'], 'spoiler-6')
  })
  ...
  await t.test('styles.css contains spoiler rules', () => {
    ...
    const cssPath = possiblePaths.find((p) => fs.existsSync(p)) ?? possiblePaths[0]
