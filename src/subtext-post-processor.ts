...
export function processSubtextParagraph(p: HTMLElement): void {
  ...
  const hasAny = lines.some(lineIsSubtext)
  if (!hasAny) return
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (lineIsSubtext(line)) {
      ...
      span.classList.add(SUBTEXT_CLASS)
...
function paragraphMightHaveSubtext(p: HTMLElement): boolean {
  for (const child of Array.from(p.childNodes)) {
    if (child.nodeType === TEXT_NODE_TYPE) {
      ...
      if (v.startsWith(MARKER) || v.includes('\n' + MARKER)) return true
    }
    if (child.nodeType === ELEMENT_NODE_TYPE && (child as Element).tagName === 'BR') {
      return true
    }
  }
  return false
...
function splitIntoLines(nodes: ChildNode[]): ChildNode[][] {
  ...
  for (const node of nodes) {
    if (node.nodeType === TEXT_NODE_TYPE) {
      ...
      for (let i = 0; i < parts.length; i++) {
        if (parts[i].length > 0) {
          ...
          const frag = doc.createTextNode(parts[i])
    ...
    } else if (node.nodeType === ELEMENT_NODE_TYPE && (node as Element).tagName === 'BR') {
...
function lineIsSubtext(line: ChildNode[]): boolean {
  for (const node of line) {
    if (node.nodeType === TEXT_NODE_TYPE) {
      return (node as Text).data.startsWith(MARKER)
    }
    if (node.nodeType === ELEMENT_NODE_TYPE && (node as Element).tagName === 'BR') continue
    return false
  }
  return false
}
...
function stripMarkerFromFirstText(nodes: Node[]): void {
  for (const node of nodes) {
    if (node.nodeType === TEXT_NODE_TYPE) {
      ...
      if (t.data.startsWith(MARKER)) {
        t.data = t.data.slice(MARKER_LEN)
      }
      return
    }
    if (node.nodeType === ELEMENT_NODE_TYPE && (node as Element).tagName === 'BR') continue
