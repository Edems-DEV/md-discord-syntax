  ...
  async onload() {
    ...
    this.registerMarkdownPostProcessor((element, _context) => {
      element.querySelectorAll('p').forEach((p) => {
        processSubtextParagraph(p as HTMLElement)
      })
      processSpoilers(element, window.document)
    })
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (mode === 'preview') {
          const preview = activeView.previewMode as unknown as MarkdownPreviewView
          const container = preview?.containerEl
          if (!container) return
          const spoilers = container.querySelectorAll('.note-flow-spoiler, .discord-syntax-spoiler')
          ...
          const cm = editor.cm
          if (!cm || !(cm instanceof EditorView) || typeof cm.dispatch !== 'function') return
          ...
          cm.dispatch({
            effects,
            selection: newSelection ? { anchor: newSelection.anchor } : undefined,
          })
        }
      },
    })
